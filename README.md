...
> 测试环境：2 vCPU / 1968 MB 的 VPS（**压测端与被测服务同机环回**）
> 上游：DeepSeek v4-flash（公网 API）；Embedding：ONNX bge-small-zh-v1.5 INT8 (512d)，进程内推理
> 测量脚本：`scripts/eval_semantic_cache.py`（语义质量）、`build/tests/recall_bench`（索引与检索）、
> `scripts/fill_cache_bench.py`（灌库内存与落盘）、`scripts/latency_bench.py`（网关自身延迟）、
> `scripts/benchmark.py`（真实上游回放）。原始输出全部在 `results/`（文件名带日期）。
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 命中 p50（真实上游回放） | 11 ms（synthetic）/ 16 ms（real） | 含进程内向量化与图检索 |
| 未命中 p50（真实上游回放） | 633 ms（synthetic）/ 614 ms（real） | 含公网 LLM API 往返，衡量的是上游不是网关 |
| 命中 p50 / 未命中 p50（mock 上游） | **6.7 ms** / 7.7 ms | 本地 mock 上游，量的是**网关自身**处理开销 |

mock 那一行是刻意补的：把上游摘掉之后，命中 6.7 ms 就是"进程内编码 + 图检索 + 输出过滤 +
HTTP 处理"的全部开销；未命中 p95 明显高于 p50（49.0 ms vs 7.7 ms），说明满缓存下的淘汰与
分配抖动集中在少数请求上。
...
| 指标 | 回源（未命中） | 命中（回放） |
|---|---|---|
| 首字节（TTFB） | 0.240 s | 0.024 s |
| 端到端 | 7.894 s | **0.033 s** |
| 客户端收到的字节 | 391,356 | 391,369（= 首次 + 注释行，去掉注释行后**逐字节相同**） |
| delta 事件数 | 1296 | 1296 |

流式命中回放的是当初记下的**原始 SSE 字节**，因此客户端侧无需任何特殊处理：
两次的事件序列与字节数完全一致，usage 完整。

### 内存与落盘

向量以 **base64 原始 float 字节**落盘（而不是 JSON 数字数组）：
...
| 灌入 10000 条 | 178.9 MB |
| 静置后（含落盘） | 179.1 MB |
| 峰值 | 223.9 MB（出现在落盘窗口内） |
| 落盘文件（10000 条） | **31.4 MB** |

落盘用 **base64 原始 float 字节**而不是 JSON 数字数组：JSON 形态每个 float 要 21 个字符
（序列化按 17 位有效数字输出），同样 1 万条的落盘会膨胀到 107.7 MB，峰值也随之抬高
——峰值那部分主要来自落盘期间要构建等量的 DOM 与 dump 字符串。base64 下 DOM 里每个向量
只是一个字符串。
...
| 指标 | 数值 |
|---|---|
| 加载 10000 条缓存（31.4 MB）到**可接受连接** | **0.58 s**（5 轮中位 0.575 s、范围 0.575–0.684 s） |
| 后台建图（10000 条真实 embedding） | 约 6 s |

建图已移到后台线程，因此它**不阻塞服务**：进程在 0.58 s 就能接受连接，建图期间
`/metrics` 的 `index_ready` 为 0，语义检索退化为线性扫描（详见下节），建图完成后自动恢复。

### 检索：索引与降级

| 规模 | 自检索 top-1 | top-3 召回 | HNSW 检索 | 暴力检索 | 加速比 | 建图 |
|---|---|---|---|---|---|---|
| 真实集 320（ONNX 编码） | **200/200 = 100%** | 100% | 192.9 µs | 253.9 µs | 1.32x | 74 ms |
| 合成 1000 | 200/200 = 100% | 99.67% | 651.3 µs | 779.9 µs | 1.20x | 1.20 s |
| 合成 5000 | 200/200 = 100% | 93.83% | 1.46 ms | 4.06 ms | 2.78x | 46.5 s |
| 合成 10000 | 200/200 = 100% | 81.67% | 1.72 ms | 8.16 ms | 4.74x | 116.8 s |

`ef_search=50` 固定时，规模增大会让束搜索覆盖不足（HNSW 固有特性）；缓存只取 top-1，
这一项不影响命中。**建图耗时对数据分布极敏感**：真实 embedding 1 万条约 6 s，
而 512 维**随机**向量（无簇结构、点与点近似等距，启发式邻居选择无从剪枝）要 117 s ——
评估建图性能必须用带簇结构的数据。

索引不可用时（启动或重建窗口）检索**不退化为"放弃语义"**，而是走 `LruStore::scan_topk()`
线性扫描：建图窗口内命中 p50 **15.2 ms**（建图完成后 HNSW 路径 6.7 ms），50/50 全部命中。
代价约 8.5 ms，换来的是这段时间不必回源（未命中一次 633 ms 且真花 token）。
...
|---|---|---|---|---|---|
| synthetic 320 × 2 轮 | 16.2% | 100% | **58.1%** | 11 ms | 633 ms |
| real 92 × 2 轮 | 5.4% | 100% | **52.7%** | 16 ms | 614 ms |

`/metrics` 汇总：命中 469 / 未命中 355 = **56.9%**，省下 **32,617 tokens**。
